...
function CollectionsChart({ periods, values, label="Cash impact" }: { periods: string[]; values: number[]; label?: string }) {
  const visible = values.map((value, index) => ({ value, period: periods[index] })).filter((x) => Math.abs(x.value) > .01);
  if (!visible.length) return <div className="empty-module compact"><h2>No monthly shift yet</h2><p>Change collection timing or discount to see the cash impact.</p></div>;
  const W=760,H=260,pad=32,max=Math.max(...visible.map((x)=>Math.abs(x.value)),1),barW=Math.max(22,Math.min(70,(W-pad*2)/visible.length-8)),gap=(W-pad*2-barW*visible.length)/Math.max(1,visible.length-1),y0=H/2;
