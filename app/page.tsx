...
export default function Home() {
  ...
  async function readVerifiedScenario(driverValues:Record<DriverKey,number>):Promise<{out:AssistantOutputs;series:number[]}|null>{
    ...
    const cashRange=protectedRange(groupCashRow.range,config.treasuryTabs),movementRange=groupMoveRow?protectedRange(groupMoveRow.range,config.treasuryTabs):"",ocfRange=groupOcfRow?protectedRange(groupOcfRow.range,config.treasuryTabs):"";
    if(!cashRange)return null;
    const ranges=[cashRange,movementRange,ocfRange].filter(Boolean),body=await sheetJson(sheetQuery(config,"read",ranges.join("|")));
    ...
    const ocfValues=ocfRange?flattenRange(values[ocfRange]).map((value)=>numberValue(value,1)):[];
    ...
    const out={closing_cash:closingCash,runway_months:closingCash!==null?(burn?Math.max(0,closingCash/burn):99):null,monthly_burn_rate:burn||null,operating_cash_flow:ocfValues[resultIndex]??null,net_cash_movement:movementValues[resultIndex]??null,cash_conversion_cycle_days:driverValues.dso+driverValues.dio-driverValues.dpo,working_capital_cash_impact:0,custom_lever_cash_impact:0,total_cash_impact:0};
  ...
  <TabsContent value="scenarios"><ScenarioLab key={`${config.ssid}:${selectedEntity}:${start}:${last}`} periods={labels.slice(start,last+1)} levers={scenarioLevers} connected={!!scan} compute={scenarioCompute} onVerify={verifyComparisonScenario} money={money}/></TabsContent>
