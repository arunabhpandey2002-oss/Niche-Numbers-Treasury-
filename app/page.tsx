...
export default function Home() {
  ...
  async function writeDrivers(){
    const builtInWrites=(Object.keys(config.driverMappings) as DriverKey[]).filter((key)=>config.driverMappings[key]&&drivers[key]!==baselineDrivers[key]).flatMap((key)=>{
      const portfolio=portfolioDriverWrites(scan,key,drivers[key]-baselineDrivers[key]);
      ...
      return range?[{range,values:[[drivers[key]]]}]:[];
    });
    const customWrites=cashLevers.filter((lever)=>lever.range&&lever.value!==lever.base).map((lever)=>({range:protectedRange(lever.range,config.treasuryTabs),values:[[lever.value]]})).filter((write)=>write.range);
    ...
    try{
      ...
      setBaselineDrivers(drivers);setCashLevers((levers)=>levers.map((lever)=>({...lever,base:lever.value})));setWriteOpen(false);setMessage("Assumptions written. Waiting for Google Sheets to recalculate…");await new Promise((resolve)=>setTimeout(resolve,800));await scanModel(false,before);return true;
  ...
  async function confirmAssistantWriteBack():Promise<AssistantOutputs|null>{
    const written=await writeDrivers();
    if(!written||!groupCashRow)return null;
    ...
    return {closing_cash:closingCash,runway_months:closingCash!==null?(burn?Math.max(0,closingCash/burn):99):null,monthly_burn_rate:burn||null,operating_cash_flow:null,net_cash_movement:movementValues[resultIndex]??null,cash_conversion_cycle_days:drivers.dso+drivers.dio-drivers.dpo,working_capital_cash_impact:0,custom_lever_cash_impact:0,total_cash_impact:0};
  }
  ...
  <TabsContent value="scenarios"><ScenarioLab periods={labels.slice(start,last+1)} levers={scenarioLevers} connected={!!scan} compute={scenarioCompute} money={money}/></TabsContent>
