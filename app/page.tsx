...
export default function Home() {
  ...
  async function writeDrivers(){
    ...
    if(!writes.length){setMessage("No changed lever has a valid writable cell in this workbook. Connect a model input first.");setWriteOpen(false);return}
    ...
    try{
      ...
      setBaselineDrivers(drivers);setCashLevers((levers)=>levers.map((lever)=>({...lever,base:lever.value})));setWriteOpen(false);setMessage("Assumptions written. Waiting for Google Sheets to recalculate…");await new Promise((resolve)=>setTimeout(resolve,800));await scanModel(false,before);
    }catch(error){setMessage(error instanceof Error?error.message:"Could not write to the model")}finally{setBusy(false)}
  }
